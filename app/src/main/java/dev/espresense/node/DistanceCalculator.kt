 ...
 *
 *   distance = 10 ^ ((refRssi - rssi) / (10 * absorption))
 *
 ...
 * exponent (ESPresense default 2.7).
 */
object DistanceCalculator {
    fun estimate(rssi: Int, refRssi: Int, absorption: Float): Double {
        if (rssi == 0) return -1.0
        val ratio = (refRssi - rssi) / (10.0 * absorption)
